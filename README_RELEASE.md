# LMA Studio v0.4.2 Release Notes

LMA Studio is a local desktop application for project-based, human-assisted LIF-MS annotation review.

## Downloads

- `LMA-Studio-v0.4.2-windows-x64.zip`: Windows x64 build. Keep the extracted folder together and run `LMAStudio.exe`.
- `LMA-Studio-v0.4.2-macos-arm64.zip`: Apple Silicon build. Open `LMA Studio.app`.
...
- The macOS desktop now asks Python to create the independent UMAP native window during the running GUI loop. It no longer depends on `window.open`, a hidden pre-created child, or custom Cocoa show/hide callbacks.
- Manual Cell pairs can be saved as pending evidence. Pending relations stay visible for later review but do not enter CSV/UMAP output, reserve an MS event, or train any model until explicitly accepted.
