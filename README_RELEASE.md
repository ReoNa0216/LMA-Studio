# LMA Studio v0.4.7 Release Notes

LMA Studio is a local desktop application for project-based, human-assisted LIF-MS annotation review.

## Downloads

- `LMA-Studio-v0.4.7-windows-x64.zip`: Windows x64 build. Keep the extracted folder together and run `LMAStudio.exe`.
- `LMA-Studio-v0.4.7-macos-arm64.zip`: Apple Silicon build. Open `LMA Studio.app`.
...
- Press `S` to toggle `Select peaks`. In `Events > Cell pair`, press `Ctrl+Enter` on Windows or `Command+Enter` on macOS to run `Save pair`. Shortcuts are ignored while typing in inputs, using a modal, or while an action is already running.
...
- Existing formal v0.4.0-v0.4.6 projects using the current adaptive two-tier LIF peak standard open from their manifest paths without migration or preprocessing reruns. Existing candidates, annotations, pending reviews, frozen models, and directory layouts remain intact.
