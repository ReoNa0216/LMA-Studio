...
macOS 压缩或跨文件系统复制可能附带 `__MACOSX/` 和 `._` 开头的 AppleDouble 元数据文件。它们不属于 LMA 的 manifest 运行依赖，也不是第二套峰表或标注。分享时可在创建压缩包的步骤排除这些元数据；不要把“整理项目”作为软件打开项目时的隐式写入。本机 CAR-T3 项目只读核查发现 124 个 `._` 文件均具有 AppleDouble 标识 `00051607`，`__MACOSX/` 内没有非 `._` 的实际数据文件。本轮保留原目录，不执行清理。格式依据：[Apple copyfile 实现](https://github.com/apple-oss-distributions/copyfile/blob/main/copyfile.c)。
