...
## Failed event-list diagnostics

New-project creation remains all-or-nothing. The matcher evaluates every source row before returning an error, then exposes a response-only diagnostic with the CSV line, `scan_start_time`, status, stable reason code, Chinese explanation, match method, candidate/matched event IDs, nearest event time, and offset. The browser downloads this as UTF-8 CSV; it is not written inside staging and therefore remains available even though the incomplete project is removed.

Failure codes distinguish `no_eligible_event_after_roster_support`, `outside_recognized_event_range`, `peak_shape_too_far`, `ambiguous_multiple_events`, and `reused_event`. Successful rows are included in the same report so the user can audit the whole list rather than only the first ten failures. UMAP coordinates, `Type`, `batch`, `CellNumber`, author labels, input paths, protocol/model hashes, and model metadata are excluded. The diagnostic changes observability only: it does not widen matching tolerance, convert ambiguous evidence into a match, or bypass the roster-review scientific gates.

## MS event-calling compatibility

The automatic/core MS760 caller remains on the established ±12 ppm trace and keeps its existing background model. A distinct roster-support lane is evaluated only when an independently supplied event-list row has no core event. That lane may inspect PC34 evidence out to ±15 ppm, but it can create a review event only on a zero-inflated trace with a measurable local-maximum distribution, a local maximum at the supplied time, sufficient prominence, and the established minimum peak distance. Strong review evidence still uses the robust `Q3 + 2.35×IQR` upper fence. A second manual-review-only gate covers resolved peaks above the 90th percentile of quiet-background local maxima; this does not lower or modify the automatic caller. Coordinate-row count, UMAP coordinates, and source identity labels never participate in threshold selection.
