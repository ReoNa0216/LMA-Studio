...
第三阶段把 QC 巡检和细胞标注放在同一时间窗口中，以“全部 / QC / 细胞”筛选和“QC anchor / 细胞二元组”手工模式区分。两类审核可任意先后，但同一个 `ms_event_id` 在当前 time model 下只能有一种 active accepted 语义；改判必须先撤销原记录。第三阶段禁用整窗批量接受。任何第三阶段接受操作都必须属于当前 frozen time model；QC 基础对齐可以来自自动模型或显式应用的 accepted-anchor refit，后者不是旧项目兼容流程的强制前置条件。

从独立 UMAP 单击事件时，主窗口按包含该事件的默认 2.5 min 网格定位，而不是用事件居中生成任意小数起点；例如 `50.075 min` 打开 `50.0-52.5 min`。若 frozen delta 使 MS 显示时间跨越网格边界，以校正后的显示时间所在网格为准。
